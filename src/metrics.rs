...
impl Metrics {
    ...
    pub fn render(&self, active_workers: usize) -> String {
        let line =
            |name: &str, v: u64| format!("{name}{{service=\"dd-fiducia-lambda-service\"}} {v}\n");
        ...
        block(
            &mut out,
            "dd_lambda_runner_invocations_total",
            "Lambda invocations handled by the runner.",
            "counter",
            g(&self.invocations_total),
        );
        block(
            &mut out,
            "dd_lambda_runner_child_spawns_total",
            "Child processes spawned by the runner.",
            "counter",
            g(&self.child_spawns_total),
        );
        block(
            &mut out,
            "dd_lambda_runner_child_reuses_total",
            "Child process reuse hits.",
            "counter",
            g(&self.child_reuses_total),
        );
        block(
            &mut out,
            "dd_lambda_runner_child_destroys_total",
            "Child processes destroyed by idle reaping or command changes.",
            "counter",
            g(&self.child_destroys_total),
        );
        block(
            &mut out,
            "dd_lambda_runner_child_exits_total",
            "Child processes that exited during invocation.",
            "counter",
            g(&self.child_exits_total),
        );
        block(
            &mut out,
            "dd_lambda_runner_invocation_timeouts_total",
            "Lambda child invocations that timed out.",
            "counter",
            g(&self.invocation_timeouts_total),
        );
        block(
            &mut out,
            "dd_lambda_runner_child_stdio_bytes_total",
            "Bytes read from child process stdio.",
            "counter",
            g(&self.child_stdio_bytes_total),
        );
        block(
            &mut out,
            "dd_lambda_runner_pool_dispatch_total",
            "Invocations dispatched to dd-container-pool over NATS.",
            "counter",
            g(&self.pool_dispatch_total),
        );
        block(
            &mut out,
            "dd_lambda_runner_pool_dispatch_failures_total",
        ...
        block(
            &mut out,
            "dd_lambda_runner_active_workers",
            "Active reusable child processes.",
            "gauge",
            active_workers as u64,
        );
        out
    }
}
